...
static HRESULT try_create_winedmo_transform(struct color_convert *impl)
{
    IMFMediaType *input_type = impl->input_type, *output_type = impl->output_type;
    MFVIDEOFORMAT *input_fmt, *output_fmt;
    ...
    if (FAILED(hr = MFCreateMFVideoFormatFromMFMediaType(input_type, &input_fmt, &input_size)))
    {
        IMFMediaType_Release(output_type);
        IMFMediaType_Release(input_type);
        return hr;
    }
    if (FAILED(hr = MFCreateMFVideoFormatFromMFMediaType(output_type, &output_fmt, &output_size)))
    {
        CoTaskMemFree(input_fmt);
        IMFMediaType_Release(output_type);
        IMFMediaType_Release(input_type);
        return hr;
    }
    ...
    status = winedmo_transform_create(MFMediaType_Video,
            (union winedmo_format *)input_fmt, input_size,
            (union winedmo_format *)output_fmt, output_size,
            &impl->winedmo_transform);
    CoTaskMemFree(input_fmt);
    CoTaskMemFree(output_fmt);
...
static HRESULT WINAPI transform_ProcessInput(IMFTransform *iface, DWORD id, IMFSample *sample, DWORD flags)
{
    ...
    status = winedmo_transform_push_input(impl->winedmo_transform, data, size, time, INT64_MIN, duration, 0);
        ...
        MFT_OUTPUT_DATA_BUFFER *samples, DWORD *status)
{
    ...
    *status = samples->dwStatus = 0;
    ...
    nt_status = winedmo_transform_get_output(impl->winedmo_transform, data, &out_size, &out_pts, &out_dur, &out_flags);
    if (!nt_status)
    {
...
HRESULT WINAPI winedmo_create_color_converter(IMFTransform **out)
{
    ...
    hr = IUnknown_QueryInterface(unknown, &IID_IMFTransform, (void **)out);
