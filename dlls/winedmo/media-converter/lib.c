    ...
    while (ptr)
    {
        *ptr = '\0';
        ...
        if (mkdir(buffer, 0777) < 0)
        {
            if (errno != EEXIST)
            {
                MEDIACONV_ERR("Failed to make directory %s. %s.", buffer, strerror(errno));
                return false;
            }
        }
        *ptr = '/';
...
static int create_file(const char *file_name)
{
    ...
    {
        MEDIACONV_WARN("Failed to open file %s with O_CREAT. %s.", file_name, strerror(errno));
        return CONV_ERROR_OPEN_FAILED;
    }

    if (futimens(fd, NULL) < 0)
    {
        MEDIACONV_WARN("Failed to set file %s timestamps. %s.", file_name, strerror(errno));
        ret = CONV_ERROR;
    }
...
bool open_file(const char *file_name, int open_flags, int *out_fd)
{
    ...
    {
        MEDIACONV_WARN("Failed to open %s with flags %d. %s.", file_name, open_flags, strerror(errno));
        return false;
    }
...
bool get_file_size(int fd, uint64_t *file_size)
{
    ...
    {
        MEDIACONV_WARN("Failed to fstat fd %d. %s.", fd, strerror(errno));
        return false;
    }
...
int create_placeholder_file(const char *file_name)
{
    ...
    MEDIACONV_ERR("Creating tag file %s.", file_name);

    if ((shader_path = getenv("STEAM_COMPAT_TRANSCODED_MEDIA_PATH")))
    {
        path_concat(path, shader_path, file_name);
        if ((ret = create_file(path)) < 0)
            MEDIACONV_ERR("Failed to create %s file, ret %d.", file_name, ret);
    }
    else
    {
        MEDIACONV_ERR("Env STEAM_COMPAT_TRANSCODED_MEDIA_PATH not set.");
...
int dump_fozdb_open(struct dump_fozdb *db, bool create, const char *file_path_env, int num_tags)
{
    ...
    {
        MEDIACONV_ERR("Env %s not set.", file_path_env);
