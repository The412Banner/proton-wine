...
static bool tag_from_ascii_bytes(uint32_t *tag, const uint8_t *ascii_bytes)
{
    ...
    *tag = strtoul(str, NULL, 16);
    if (errno != 0)
    {
        MEDIACONV_ERR("Failed to convert string \"%s\" to tag. %s.", str, strerror(errno));
        return false;
    }
...
static bool hash_from_ascii_bytes(struct fozdb_hash *hash, const uint8_t *ascii_bytes)
{
    ...
    for (i = 0; i < ARRAY_SIZE(hash->hash); ++i)
    {
        ...
        *hash_part = strtoul(str, NULL, 16);
        ...
        if (errno != 0)
        {
            MEDIACONV_ERR("Failed to convert string \"%s\" to hash part %u. %s.", str, 4 - i, strerror(errno));
            return false;
        }
    }
        ...
        uint32_t *out_tag, struct fozdb_hash *out_hash, struct payload_header *out_header)
{
    ...
    {
        MEDIACONV_ERR("Failed to read entry name and header.");
        return CONV_ERROR_READ_FAILED;
    }
...
static bool fozdb_seek_to_next_entry(struct fozdb *db, struct payload_header *header, bool *truncated)
{
    ...
    {
        MEDIACONV_ERR("Failed to seek to next entry. %s. "
    ...
    if (file_size && data_offset + header->size > file_size)
    {
        ...
        MEDIACONV_WARN("Entry data larger than file, truncating database here. "
            "Entry data offset %#"PRIx64", size %#x, file size %#"PRIx64".",
...
static bool fozdb_write_entry_name(struct fozdb *db, uint32_t tag, struct fozdb_hash *hash)
{
    ...
    {
        MEDIACONV_ERR("Failed to write entry name.");
        return false;
    }
        ...
        struct fozdb_key *key, struct payload_header *header, uint64_t entry_data_offset)
{
    ...
    {
        MEDIACONV_ERR("Failed to seek file to write_pos.");
    ...
    {
        MEDIACONV_ERR("Failed to write entry header.");
        return CONV_ERROR_WRITE_FAILED;
    }
    ...
    for (read_offset = entry_data_offset; read_offset < entry_end; read_offset += read_size)
    {
        ...
        {
            MEDIACONV_ERR("Failed to seek to read offset. %s.", strerror(errno));
            free(buffer);
            return CONV_ERROR_SEEK_FAILED;
        }
        if ((read_size = read(db->file, buffer, to_read)) < 0)
        {
            MEDIACONV_ERR("Failed to read entry data. %s.", strerror(errno));
            free(buffer);
            return CONV_ERROR_READ_FAILED;
        }
        if (read_size == 0)
        ...
        if (lseek(db->file, db->write_pos, SEEK_SET) < 0)
        {
            MEDIACONV_ERR("Failed to seek to write_pos. %s.", strerror(errno));
            free(buffer);
            return CONV_ERROR_SEEK_FAILED;
        }
        if (!complete_write(db->file, buffer, read_size))
        {
            MEDIACONV_ERR("Failed to write entry data to write_pos.");
            free(buffer);
            return CONV_ERROR_WRITE_FAILED;
        }
...
int fozdb_create(const char *file_name, int open_flags, bool read_only, uint32_t num_tags, struct fozdb **out)
{
    ...
    MEDIACONV_DBG("file_name %s, open_flags %d, read_only %d, num_tags %u, out %p.",
    ...
    {
        MEDIACONV_ERR("Failed to prepare fozdb, ret %d.", ret);
        fozdb_release(db);
        return ret;
    }

    MEDIACONV_INFO("Created fozdb %p from %s.", db, file_name);
...
int fozdb_prepare(struct fozdb *db)
{
    ...
    MEDIACONV_DBG("db %p, file_name %s, read_only %d, num_tags %u.",
    ...
    if (!file_size)
    {
        ...
        {
            MEDIACONV_ERR("Failed to write file header.");
            return CONV_ERROR_WRITE_FAILED;
        }
    ...
    while (db->write_pos < file_size)
    {
        ...
        MEDIACONV_INFO("Got entry: tag %u, hash %s, offset %#"PRIx64", size %#x, crc %#x.",
                tag, format_hash(&hash), offset, header.size, header.crc);

        /* Insert entry to hash table. */
        if (tag >= db->num_tags)
        {
            MEDIACONV_WARN("Invalid tag %u.", tag);
        ...
        uint64_t offset, uint8_t *buffer, size_t size, size_t *read_size, bool with_crc)
{
    ...
    MEDIACONV_DBG("db %p, file_name %s, tag %u, hash %s, offset %#"PRIx64", buffer %p, size %zu, read_size %p, with_crc %d.",
    ...
    *read_size = 0;
    ...
    {
        MEDIACONV_ERR("Failed to read entry data.");
        return CONV_ERROR_READ_FAILED;
    }
    *read_size = to_copy;
        ...
        void *data_src, data_read_callback read_callback, bool with_crc)
{
    ...
    MEDIACONV_DBG("db %p, file_name %s, tag %u, hash %s, data_src %p, read_callback %p, with_crc %d.",
            db, db->file_name, tag, format_hash(hash), data_src, read_callback, with_crc);

    if (tag >= db->num_tags)
    {
        MEDIACONV_ERR("Invalid tag %u.", tag);
        return CONV_ERROR_INVALID_TAG;
    }
    ...
    if (lseek(db->file, db->write_pos, SEEK_SET) < 0)
    {
        MEDIACONV_ERR("Failed to seek file to write_pos %#"PRIx64".", db->write_pos);
        return CONV_ERROR_SEEK_FAILED;
    }
    ...
    while ((ret = read_callback(data_src, buffer, BUFFER_COPY_BYTES, &read_size)) == CONV_OK)
    {
        ...
        {
            MEDIACONV_ERR("Data too large. Fossilize format only supports 4 GB entries.");
            free(buffer);
            return CONV_ERROR;
        }
        ...
        if (!complete_write(db->file, buffer, read_size))
        {
            MEDIACONV_ERR("Failed to write entry data.");
            free(buffer);
            return CONV_ERROR_WRITE_FAILED;
        }
    ...
    {
        MEDIACONV_ERR("Failed to read data from data src, ret %d.", ret);
        return ret;
    }
    ...
    {
        MEDIACONV_ERR("Failed to seek back to entry header. %s.", strerror(errno));
        return CONV_ERROR_SEEK_FAILED;
    }
    header.size = size;
    ...
    MEDIACONV_INFO("Wrote entry: tag %u, hash %s, offset %#"PRIx64", size %#x, crc %#x.",
            tag, format_hash(hash), offset, header.size, header.crc);
...
int fozdb_discard_entries(struct fozdb *db, struct rb_tree *to_discard)
{
    ...
    MEDIACONV_DBG("db %p, file_name %s, to_discard %p.", db, db->file_name, to_discard);

    /* Rewind the file and clear the entry tables. */
    if (lseek(db->file, 0, SEEK_SET) < 0)
    {
        MEDIACONV_ERR("Failed to seek to file start. %s.", strerror(errno));
        return CONV_ERROR_SEEK_FAILED;
    }
    rb_destroy(&db->entries, fozdb_entry_destroy, NULL);
    ...
    {
        MEDIACONV_ERR("Failed to truncate file. %s.", strerror(errno));
        return CONV_ERROR;
    }
