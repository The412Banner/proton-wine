...
#define MEDIACONV_ERR(fmt, ...) ERR(fmt "\n", ## __VA_ARGS__)
#define MEDIACONV_WARN(fmt, ...) WARN(fmt "\n", ## __VA_ARGS__)
#define MEDIACONV_INFO(fmt, ...) TRACE(fmt "\n", ## __VA_ARGS__)
#define MEDIACONV_DBG(fmt, ...) TRACE(fmt "\n", ## __VA_ARGS__)
#define GST_WARNING(fmt, ...) MEDIACONV_WARN(fmt, ## __VA_ARGS__)
typedef struct media_buffer media_buffer_t; /* not used */
extern size_t media_buffer_extract(media_buffer_t*,size_t,void*,size_t); /* not used */
...
struct media_buffer_reader
{
    media_buffer_t *buffer; /* No ref here, no need to unref. */
    size_t offset;
};
...
static inline void media_buffer_reader_init(struct media_buffer_reader *reader, media_buffer_t *buffer)
{
    reader->buffer = buffer; /* No ref here, so no need to unref. */
    reader->offset = 0;
}

static inline int media_buffer_reader_read(void *data_reader, uint8_t *buffer, size_t size, size_t *read_size)
{
    struct media_buffer_reader *reader = data_reader;
    ...
    *read_size = media_buffer_extract(reader->buffer, reader->offset, buffer, size);
