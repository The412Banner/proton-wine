 ...
 *
 * Load a unix .so library by name, searching build_dir and dll_paths[]
 * (which includes $PREFIX/lib/wine on Winlator-bionic containers). Used
 * by the MemoryWineLoadUnixLibByName NtQueryVirtualMemory class so that a
 * dropped-in arm64ec PE DLL (e.g. libarm64ecfex.dll) can pull in its
 * native companion unixlib (libarm64ecfex.so) at runtime.
 */
NTSTATUS load_unixlib_by_name( const UNICODE_STRING *nt_name, void **handle_ret )
{
    ...
    if (i < len)  /* explicit path */
    {
        ...
        if (!nt_to_unix_file_name( &attr, &file, FILE_OPEN ))
    ...
    if (build_dir)
    {
        ptr = prepend_build_dir_path( file + pos, ".so", "", "/dlls" );
        strcpy( ext, ".so" );
